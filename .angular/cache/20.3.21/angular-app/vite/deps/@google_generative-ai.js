...
function constructModelRequest(model, task, apiKey, stream, body, requestOptions) {
  return __async(this, null, function* () {
    ...
    return {
      url: url.toString(),
      fetchOptions: Object.assign(Object.assign({}, buildFetchOptions(requestOptions)), { method: "POST", headers: yield getHeaders(url), body })
    };
  });
}
function makeModelRequest(_0, _1, _2, _3, _4) {
  return __async(this, arguments, function* (model, task, apiKey, stream, body, requestOptions = {}, fetchFn = fetch) {
    const { url, fetchOptions } = yield constructModelRequest(model, task, apiKey, stream, body, requestOptions);
...
function batchEmbedContents(apiKey, model, params, requestOptions) {
  return __async(this, null, function* () {
    ...
    const response = yield makeModelRequest(model, Task.BATCH_EMBED_CONTENTS, apiKey, false, JSON.stringify({ requests: requestsWithModel }), requestOptions);
    return response.json();
  });
}
